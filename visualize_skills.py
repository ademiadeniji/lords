...
flags.DEFINE_string('env_name', 'FetchPushPrimitives-v1', 'Name of an environment')
flags.DEFINE_boolean('object', False, 'Object-Centric demos')
flags.DEFINE_integer(
    'path_len', 50, 'Path length'
)
flags.DEFINE_integer(
...
def evaluate_policy():
    ...
    path_len = FLAGS.path_len
    num_demos = FLAGS.num_demos
    epsilon = 0.75
    epsilon_greedy = False
    state_noise = False
    action_noise = False
    skill_randomization = False
    plot_actions = False
    ...
    if make_video:
        ...
        color_wheel = ['b', 'r', 'g', 'c', 'm', 'indigo', 'brown', 'gray']
        for i in range(dim_z):
            for runs in range(num_demos):
                xs_list = []
                ys_list = []
                actions_list = {new_list: [] for new_list in range(action_spec.shape[0])} 
                ...
                with imageio.get_writer(video_filename, fps=60) as video:
                    for _ in range(num_eps):
                        ...
                        if has_object:
                            print("{} {}".format(_time_step.observation["observation"][3], _time_step.observation["observation"][4]))
                        else:
                            print("{} {}".format(_time_step.observation["observation"][0], _time_step.observation["observation"][1]))
                        ...
                        while steps < path_len:
                            ...
                            if (steps+1) % print_interval == 0: 
                                if has_object:
                                    print("{} {}".format(_time_step.observation["observation"][3], _time_step.observation["observation"][4]))
                                    xs_list.append(_time_step.observation["observation"][3])
                                    ys_list.append(_time_step.observation["observation"][4])
                                else:
                                    print("{} {}".format(_time_step.observation["observation"][0], _time_step.observation["observation"][1]))
                    ...
                    if "Fetch" in FLAGS.env_name: 
                        ...
                        plt.xlim(-10, 10)
                        plt.ylim(-10, 10)
                    plt.plot(xs_list, ys_list, color_wheel[i%len(color_wheel)], label='skill '+str(i+1))
        ...
        for i in range(0, dim_z, 1):
            ...
            plt.xlim(-100, 100)
            plt.ylim(-100, 100)
            plt.plot(xs_list, ys_list, label='skill'+str(i))
            plt.legend(loc="upper left")
        plt.savefig(skill_plot_filename)
