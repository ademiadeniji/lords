...
@gin.configurable
class HrlDynamicEpisodeDriver(object):
  ...
  def __init__(self,
               ...
               num_episodes=1):
    """Creates a HrlDynamicEpisodeDriver.
  ...
  def _loop_body_fn(self):
    ...
    def loop_body(counter, skill_counter, z, time_step, high_level_policy_state, low_level_policy_state):
      ...
      z = tf.cond(skill_counter % self.low_level_policy.steps_per_option == 0, 
        lambda: self.high_level_policy.action(time_step, high_level_policy_state).action, lambda: z)
      ...
      return [counter, skill_counter, z, next_time_step, high_level_policy_state, low_level_policy_state]

    return loop_body

  def run(self,
          time_step=None,
          high_level_policy_state=None,
          low_level_policy_state=None,
          num_episodes=None,
  ...
  def _run(self,
    ...
    [_, _, _, time_step, high_level_policy_state, low_level_policy_state] = tf.while_loop(
        cond=self._loop_condition_fn(num_episodes),
        body=self._loop_body_fn(),
        loop_vars=[counter, skill_counter, z, time_step, high_level_policy_state, low_level_policy_state],
        back_prop=False,
        parallel_iterations=1,
        maximum_iterations=maximum_iterations,
        name='driver_loop')
    return time_step, high_level_policy_state, low_level_policy_state
