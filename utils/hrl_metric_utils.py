...
from . import hrl_dynamic_episode_driver
...
def eager_compute(metrics,
  ...
  *NOTE*: Because placeholders are not compatible with Eager mode we can not use
  ...
  time_step = environment.reset()
  high_level_policy_state = high_level_eval_policy.get_initial_state(environment.batch_size)
  low_level_policy_state = low_level_eval_policy.get_initial_state(environment.batch_size)

  driver = hrl_dynamic_episode_driver.HrlDynamicEpisodeDriver(
      environment,
      high_level_eval_policy,
      low_level_eval_policy,
      observers=metrics,
  ...
  if use_function:
    common.function(driver.run)(time_step, high_level_policy_state, low_level_policy_state)
  else:
    driver.run(time_step, high_level_policy_state, low_level_policy_state)
