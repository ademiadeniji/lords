...
@gin.configurable
class LatentCriticNetwork(network.Network):
  ...
  def __init__(self,
    ...
    Raises:
      ValueError: If `observation_spec` or `action_spec` contains more than one
        observation.
    """
    self._handcraft_obs_critic = handcraft_obs_critic
    self._num_skills = num_skills
    super(LatentCriticNetwork, self).__init__(
  ...
  def call(self, inputs, step_type=(), network_state=(), training=False):
    ...
    if self._handcraft_obs_critic:
        skill = observations[:, -self._num_skills:]
        observation = observations[:, :-self._num_skills]
